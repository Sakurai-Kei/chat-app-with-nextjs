...
export default function MemberList(props: MemberListProps) {
  ...
  if (!showMember) {
    return (
      <div ref={closeMemberList} className="flex transition ease-in-out">
        <button
          onClick={() => {
            closeMemberList.current!.className =
              "flex transition ease-in-out translate-x-full";
            setTimeout(() => {
              showMemberComponent();
            }, 200);
          }}
          className="w-12 flex justify-center items-center bg-slate-800 hover:bg-slate-700 hover:animate-bounceLeft"
        >
          <svg
            width="24"
            height="24"
    ...
    return (
      <div
        ref={openMemberList}
        className="flex transition ease-in-out flex-col flex-shrink-0 w-full h-screen z-10 absolute md:relative md:z-0 text-white md:w-1/4 border-l border-gray-300 bg-slate-600"
      >
        <div className="flex items-center h-16 border-b border-gray-300 px-4">
          <div className="flex flex-col gap-2">
            ...
            <div className="text-xs leading-none text-gray-300">Members</div>
          </div>
          <button
            onClick={() => {
              openMemberList.current!.className =
                "translate-x-full flex transition ease-in-out flex-col flex-shrink-0 w-full h-screen z-10 absolute md:relative md:z-0 text-white md:w-1/4 border-l border-gray-300 bg-slate-600";
              setTimeout(() => {
                showMemberComponent();
              }, 200);
            }}
              ...
              return (
                ...
                >
                  ...
                  <div className="ml-2">
                    ...
                    <p className="text-sm text-gray-300">{member.about}</p>
                  </div>
                </div>
              );
            })}
